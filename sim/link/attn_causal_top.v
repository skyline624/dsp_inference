`timescale 1ns/1ps
// attn_causal_top - one node (top.v LINK_SS) + the causal attention sequencer,
// wired over a pair of ss_link fifos. Test seeds the node SDRAM (weights) and the
// sequencer KV-cache (earlier positions), drives one causal step, reads attn_out.
module attn_causal_top #(
    ...
    input  wire [5:0]     pos,
    input  wire [16*4-1:0] cos_q15,
    input  wire [16*4-1:0] sin_q15,
    ...
    // command fifo : seq writes, node reads
    ...
    // response fifo : node writes, seq reads
