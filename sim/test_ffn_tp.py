"""Autonomous TENSOR-PARALLEL FFN over 2 brick nodes (no PC).

The on-chip ffn_tp_seq splits the FFN across 2 nodes (hidden=128 -> 64/node):
rmsnorm broadcast, W1/W3 row-parallel, silu*  per node, W2 column-parallel,
all-reduce + residual via vec_alu. Weights split across the two SDRAMs. Validated
within quantization tolerance vs the full float FFN.
"""
...
async def test_ffn_tp(dut):
    ...
    for _ in range(800000):
        ...
        assert False, "ffn_tp_seq never finished"
    ...
    dut._log.info(f"  autonomous TP-FFN (2 nodes): max_err={err:.4f} ({100*err/ma:.1f}% of range)")
    assert err/ma < 0.35, f"TP-FFN output too far from reference ({err})"

    dut._log.info(
        "FFN TENSOR-PARALLEL (autonomous) PASS: on-chip sequencer split the FFN "
        "across 2 nodes (W1/W3 row-parallel, W2 col-parallel + all-reduce), zero PC."
    )
